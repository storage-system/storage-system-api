...
export class GetStyleUseCase {
  ...
  async execute(anInput: CreateStyleUseCaseRequest): Promise<GetStyleUseCaseResponse> {
    const style = await this.styleRepository.findById(anInput.styleId)

    if (!style) {
      throw ResourceNotFoundException.with('Estilização', new UniqueEntityID(anInput.styleId));
    }
