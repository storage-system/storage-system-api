...
export class CompanyPresenter {
  ...
  static fromAggregate(aCompany: Company, user: User[]) {
    const userProps = user.length > 0 ? user.map((user) => user.toJSON()) : []

    return new CompanyPresenter(aCompany.toJSON(), userProps)
  }
}
